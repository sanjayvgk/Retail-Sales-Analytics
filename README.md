...
The project now includes an executable Python pipeline, the cleaned Sample Superstore dataset, generated analytical outputs, MySQL dimension/fact exports, a database load script, automated data-quality tests, a Power BI-ready dataset and DAX measures, dashboard build documentation, and an evidence-based business report.
...
Retail-Sales-Analytics/
├── README.md
├── data/                         # Raw, processed, and MySQL-ready datasets
├── notebooks/                    # Python notebooks, if included
├── powerbi/
│   ├── dashboard_guide.md        # Step-by-step Power BI dashboard guide
│   └── measures.dax              # Core DAX measures
├── reports/
│   └── business_report.md        # Senior business analyst report
├── sql/
│   ├── schema.sql                # MySQL database schema and reporting view
│   ├── load_data.sql             # Loads generated CSV exports into MySQL
│   └── queries.sql               # SQL query library for analysis
├── src/                          # Python source code
├── scripts/
│   ├── run_pipeline.py           # End-to-end Python analysis
│   └── build_database_exports.py # Creates MySQL-ready dimension/fact CSVs
├── tests/
│   └── test_pipeline.py          # Automated data-quality tests
...
The project uses the public Sample Superstore CSV. The source file is included under `data/raw/superstore.csv` for reproducibility. The original public dataset is available from the GitHub repository linked in the project documentation. The raw source contains 10,800 rows; the pipeline removes 504 exact duplicate rows and produces the standard 9,994-row analysis dataset.
...
python scripts/build_database_exports.py
```

This creates the cleaned dataset, KPI/EDA CSVs, 12 charts, and MySQL-ready exports.

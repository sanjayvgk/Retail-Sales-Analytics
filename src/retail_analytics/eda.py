...
def summarize_metrics(df: pd.DataFrame) -> dict[str, float | int]:
    ...
    return {
        ...
        "profit_margin": float(df["Profit"].sum() / df["Sales"].sum()) if "Sales" in df.columns and df["Sales"].sum() else 0.0,
        "average_discount": float(df["Discount"].mean()) if "Discount" in df.columns else 0.0,
...
def monthly_trends(df: pd.DataFrame) -> pd.DataFrame:
    ...
    work = df.copy()
    work["Order Date"] = pd.to_datetime(work["Order Date"], errors="coerce")
    monthly = work.set_index("Order Date").resample("ME").agg({"Sales": "sum", "Profit": "sum"}).reset_index()
